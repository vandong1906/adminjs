...
export class AttributeGroupHandler {
  ...
  static async getAttributeGroup(id: string) {
    try {
  ...
  static async createAttributeGroup(data: any) {
    try {
  ...
  static async updateAttributeGroup(id: string, data: any) {
    try {
      const group = await Models.AttributeGroup.findByPk(id);
      if (!group) {
        return { success: false, error: 'Attribute group not found' };
  ...
  static async deleteAttributeGroup(id: string) {
    try {
      const group = await Models.AttributeGroup.findByPk(id);
      if (!group) {
        return { success: false, error: 'Attribute group not found' };
